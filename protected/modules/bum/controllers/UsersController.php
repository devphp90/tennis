    ...
    public function actionActivate($acKey) {
        ...
        if ($modelUsersData === NULL) {
            ...
            if ($model->active) {
                ...
                # set the guiders for user
                # insert into users_guiders_flag table
                $pages = GuidersPage::model()->findAll();
                $user_guider = new UsersGuidersFlag;

                if (!empty($pages)) {
                    foreach ($pages as $page) {
                        $user_guider->isNewRecord = true;
                        $user_guider->primaryKey = NULL;
                        $user_guider->user_id = $model->id;
                        $user_guider->flag = 0;
                        $user_guider->guiders_page_id = $page->id;
                        $user_guider->save();
                    }
                }
